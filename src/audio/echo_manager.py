...
class EchoCancellationManager:
    ...
    def process_microphone_audio(self, input_audio):
        ...
        # 执行回声消除 - 添加异常处理
        try:
            cleaned_audio = self.echo_canceller.process_audio(
                input_audio, 
    ...
    def _safety_check_and_mix(self, original_audio, cleaned_audio):
        ...
        # 检查是否过度抑制
        if (cleaned_rms < original_rms * self.min_energy_ratio and 
            original_rms > self.min_original_rms):
            ...
            if self.frame_count % 100 == 0:
                self._log_debug(f"Frame {self.frame_count}: 检测到过度抑制，混合原始音频 "
    ...
    def _mix_audio(self, original_audio, processed_audio, original_ratio):
        ...
        mixed_audio = (original_audio.astype(np.float32) * original_ratio + 
                      processed_audio.astype(np.float32) * processed_ratio)
