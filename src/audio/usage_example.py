...
# 创建管理器
echo_manager = EchoCancellationManager(
...
def advanced_configuration_example():
    ...
    # 安静环境配置 - 更激进的回声消除
    quiet_environment = EchoCancellationManager(
        enable_echo_cancellation=True,
        enable_debug=False
    )
    quiet_environment.set_parameters(
        min_energy_ratio=0.05,  # 更低的阈值
        mix_ratio=0.2           # 更少的原始音频混合
    )
    
    # 嘈杂环境配置 - 更保守的回声消除
    noisy_environment = EchoCancellationManager(
        enable_echo_cancellation=True,
        enable_debug=True
    )
    noisy_environment.set_parameters(
        min_energy_ratio=0.3,   # 更高的阈值
        mix_ratio=0.5           # 更多的原始音频混合
    )
    
    # 调试模式配置 - 详细的日志输出
    debug_mode = EchoCancellationManager(
        enable_echo_cancellation=True,
        enable_debug=True
    )
    debug_mode.set_parameters(
        debug_interval=50       # 更频繁的调试输出
    )
